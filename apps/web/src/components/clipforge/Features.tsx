    ...
    extra: (
      <div className="mt-6 flex items-end gap-1.5" aria-hidden="true">
        {[35, 60, 45, 80, 50, 90, 65, 40, 75, 55, 85, 62, 45, 70].map((h, i) => (
          <motion.div
            key={i}
            className="w-full origin-bottom rounded-t-md bg-gradient-to-t from-[#EA4C89]/40 to-[#EA4C89]/80 transition-colors duration-300 group-hover:from-[#EA4C89]/60 group-hover:to-[#EA4C89]"
            style={{ height: `${h}%` }}
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f, i) => (
            ...
            >
              ...
              <ArrowUpRight
                size={18}
                className="absolute top-5 right-5 text-[#EA4C89] opacity-0 transition-all duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:opacity-100"
              />
            </motion.div>
          ))}
        </div>
