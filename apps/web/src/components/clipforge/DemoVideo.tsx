              ...
              >
                ...
                {!playing && (
                  <div className="absolute inset-0 grid place-items-center bg-black/25">
                    <div className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-[#0D0C22] shadow-lg">
                      ▶ Hover untuk memutar
                    </div>
                  </div>
                )}
            ...
            {highlights.map((h, i) => (
              <Reveal key={h.title} delay={i * 0.1}>
                <div className="glass-card flex gap-4 rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1">
                  <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${h.iconBg} text-[#0D0C22]`}>
                    <h.icon size={22} />
                  </div>
                  <div>
                    <h3 className="font-semibold text-[#0D0C22]">{h.title}</h3>
                    <p className="mt-1 text-sm leading-relaxed text-[#6E6D7A]">{h.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
