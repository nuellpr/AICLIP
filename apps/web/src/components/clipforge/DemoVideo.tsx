      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <Reveal className="mx-auto mb-14 max-w-2xl text-center">
          ...
          <h2 className="text-3xl font-extrabold tracking-tight sm:text-5xl">
            Lihat Cara Kerja AI{" "}
            <span className="text-gradient-blue">Memotong & Mengatur Subtitle</span>
          </h2>
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[minmax(0,420px)_1fr] lg:gap-16">
          <Reveal className="mx-auto w-full max-w-[340px]">
            <div className="relative">
              <div className="pointer-events-none absolute -inset-4 rounded-[3rem] bg-gradient-to-b from-cyan-400/20 via-purple-500/15 to-transparent blur-2xl" />

              <div
                className="relative overflow-hidden rounded-[2.2rem] border-2 border-white/10 bg-[#0B0D16] shadow-[0_25px_80px_-20px_rgba(0,0,0,0.8)]"
                style={{ aspectRatio: "9/16" }}
              >
                <div className="absolute top-3 left-1/2 z-10 h-5 w-24 -translate-x-1/2 rounded-full bg-black border border-white/10" />
                <video
                  ref={videoRef}
                  src="/tutorial.mp4"
                ...
                />
                <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#05060B] via-[#05060B]/70 to-transparent p-4 pt-10">
                  <div className="text-sm font-bold leading-snug text-white">
                    "Yang bikin orang <span className="text-cyan-300">STOP scroll…</span>"
                  </div>
                  <div className="mt-0.5 text-[10px] tracking-wider text-purple-300 uppercase">
                    Subtitle Hormozi • Word-by-word
                  </div>
                </div>
                {!playing && (
                  <div className="absolute inset-0 grid place-items-center bg-black/25">
                    <div className="rounded-full border border-white/25 bg-black/60 px-4 py-2 text-xs text-white">
                      ▶ Hover untuk memutar
                    </div>
                  </div>
                )}
                <div className="absolute top-12 right-3 rounded-full bg-cyan-400 px-2.5 py-1 text-[10px] font-extrabold text-black">
                  FULL HD 1080P
                </div>
              </div>
            </div>
            ...
            {highlights.map((h, i) => (
              <Reveal key={h.title} delay={i * 0.1}>
                <div className="glass-card flex gap-4 rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-400/20 to-purple-500/20 text-cyan-300">
                    <h.icon size={22} />
                  </div>
                  <div>
                    <h3 className="font-semibold text-white">{h.title}</h3>
                    <p className="mt-1 text-sm leading-relaxed text-slate-400">{h.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
