...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#060609] text-white flex flex-col justify-between selection:bg-cyan-500 selection:text-black">
      ...
      {/* Official Google Sign-In Account Chooser Modal (Image 2) */}
      {showGoogleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-[#18181c] border border-white/10 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl relative">
            ...
            <div className="p-6 pb-4 border-b border-white/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-sm text-white leading-tight">Login dengan Google</h3>
                  <p className="text-[11px] text-gray-400">Lanjutkan ke <span className="text-cyan-400 font-medium">klipaja.id</span></p>
                </div>
              </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                <h4 className="text-2xl font-bold text-white mb-1">Pilih akun</h4>
                <p className="text-xs text-gray-400">Lanjutkan ke <span className="text-cyan-400 font-bold">klipaja.id</span></p>
              </div>
              ...
              <div className="space-y-2">
                {savedGoogleAccounts.map((acc, idx) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-cyan-400/50 transition-all text-left group"
                  >
                    <div className="flex items-center gap-3.5">
                      <div className="w-10 h-10 rounded-full bg-cyan-500/20 text-cyan-400 font-bold text-sm flex items-center justify-center border border-cyan-500/30 overflow-hidden">
                        {acc.name.charAt(0)}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-white group-hover:text-cyan-400 transition-colors">{acc.name}</p>
                ...
                {/* Option: Use Custom Google Email */}
                {!showCustomInput ? (
                  <button
                    type="button"
                    onClick={() => setShowCustomInput(true)}
                    className="w-full flex items-center gap-3.5 p-3.5 rounded-2xl bg-transparent hover:bg-white/5 border border-dashed border-white/20 transition-all text-left text-xs font-semibold text-gray-300 hover:text-white"
                  >
                  ...
                  <form
                    onSubmit={(e) => {
                      e.preventDefault();
                      if (customGoogleEmail) executeGoogleAuth(customGoogleEmail);
                    }}
                    className="p-3.5 rounded-2xl bg-white/5 border border-cyan-500/40 space-y-3"
                  >
                    <label className="text-xs font-bold text-cyan-400 block">Masukkan Email Google Anda:</label>
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="contoh: studio@gmail.com"
            ...
            {/* Modal Footer */}
            <div className="p-4 bg-black/40 border-t border-white/10 flex items-center justify-between text-[11px] text-gray-500">
              <span>Indonesia</span>
              <div className="flex items-center gap-3">
                <Link href="#" className="hover:text-gray-400">Bantuan</Link>
                <Link href="#" className="hover:text-gray-400">Privasi</Link>
                <Link href="#" className="hover:text-gray-400">Persyaratan</Link>
              </div>
            </div>
          </div>
        </div>
