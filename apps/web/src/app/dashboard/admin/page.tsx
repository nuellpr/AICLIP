  ...
  if (currentUser && currentUser.role !== 'ADMIN') {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6 space-y-4">
        ...
        <h2 className="text-2xl font-black text-[#0D0C22]">Akses Ditolak (Hanya Khusus Admin)</h2>
        <p className="text-sm text-[#6E6D7A] max-w-md">
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-black/10 pb-6">
        ...
        <button
          ...
          disabled={loading}
          className="self-start md:self-auto flex items-center gap-2 bg-white hover:bg-[#F8F7F4] text-[#0D0C22] border border-black/10 px-4 py-2 rounded-xl text-xs font-bold transition-all disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="rounded-3xl glass-card p-6 border border-black/10 relative overflow-hidden group hover:border-[#EA4C89]/40 transition-all">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-[#6E6D7A] uppercase tracking-wider">Total Pengguna</p>
            <div className="p-2.5 rounded-2xl bg-[#FDE3E1]/60 text-[#EA4C89] border border-[#FDE3E1]">
              <Users className="w-5 h-5" />
            </div>
          </div>
          <p className="text-3xl font-black text-[#0D0C22] mt-4">{stats?.totalUsers ?? 0}</p>
          <p className="text-[11px] text-[#6E6D7A] mt-1">Akun terdaftar dalam sistem</p>
        </div>

        <div className="rounded-3xl glass-card p-6 border border-black/10 relative overflow-hidden group hover:border-[#7C3AED]/40 transition-all">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-[#6E6D7A] uppercase tracking-wider">Proyek Diproses</p>
            <div className="p-2.5 rounded-2xl bg-[#F3E8FF] text-[#7C3AED] border border-[#E9D5FF]">
              <Video className="w-5 h-5" />
            </div>
          </div>
          <p className="text-3xl font-black text-[#0D0C22] mt-4">{stats?.totalProjects ?? 0}</p>
          <p className="text-[11px] text-[#6E6D7A] mt-1">{stats?.totalClips ?? 0} Klip berhasil dirender</p>
        </div>

        <div className="rounded-3xl glass-card p-6 border border-black/10 relative overflow-hidden group hover:border-[#14532D]/40 transition-all">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-[#6E6D7A] uppercase tracking-wider">Total Pendapatan</p>
            <div className="p-2.5 rounded-2xl bg-[#DBF3E8] text-[#14532D] border border-[#DBF3E8]">
              <DollarSign className="w-5 h-5" />
            </div>
          </div>
          <p className="text-3xl font-black text-[#0D0C22] mt-4">{formatRupiah(stats?.totalRevenue ?? 0)}</p>
          <p className="text-[11px] text-[#6E6D7A] mt-1">Transaksi settlement</p>
        </div>

        <div className="rounded-3xl glass-card p-6 border border-black/10 relative overflow-hidden group hover:border-[#854D0E]/40 transition-all">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-[#6E6D7A] uppercase tracking-wider">Kesehatan Redis</p>
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <p className="text-xl font-black text-[#0D0C22] uppercase">{queueData?.redis?.status || 'READY'}</p>
          </div>
          <p className="text-[11px] text-[#6E6D7A] mt-1 truncate">{queueData?.redis?.url || '127.0.0.1:6379'}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 rounded-3xl glass-panel p-6 border border-black/10 flex flex-col justify-between">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-lg font-bold text-[#0D0C22] flex items-center gap-2">
                <Activity className="w-5 h-5 text-[#EA4C89]" />
                <span>Pemrosesan Video (7 Hari Terakhir)</span>
              </h2>
              <p className="text-xs text-[#6E6D7A] mt-0.5">Grafik jumlah klip video yang diproses per hari</p>
            </div>
            <span className="text-[11px] text-[#6E6D7A] bg-[#F8F7F4] px-3 py-1 rounded-full border border-black/5">
              Realtime Activity
            </span>
          </div>

          {/* Bar Chart Visualization */}
          <div className="flex items-end justify-between gap-3 h-48 pt-6 px-2 border-b border-black/10 pb-2">
            {stats?.dailyStats?.map((d, index) => {
              ...
              return (
                <div key={index} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end">
                  ...
                  ></div>
                  <span className="text-[10px] text-[#6E6D7A] font-semibold mt-1">{dayName}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="lg:col-span-5 rounded-3xl glass-panel p-6 border border-black/10 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-[#0D0C22] flex items-center gap-2">
          ...
          <div className="space-y-4">
            {queueData?.queues?.map((q, idx) => (
              <div key={idx} className="p-4 rounded-2xl bg-[#F8F7F4] border border-black/10 space-y-3">
                <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-4 gap-2 text-center text-[10px]">
                  ...
                  <div className="p-2 rounded-xl bg-[#FDE3E1] border border-[#FDE3E1]">
                    <p className="text-[#6E6D7A] font-semibold">Gagal</p>
      ...
      <div className="space-y-4 border-t border-black/10 pt-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-[#0D0C22]">Manajemen Akun Pengguna & Kredit</h2>
            <p className="text-xs text-[#6E6D7A]">Kelola kredit AI dan hak akses admin untuk setiap akun.</p>
          </div>
        </div>
        ...
        <div className="overflow-x-auto rounded-2xl border border-black/10 glass-card bg-white">
          <table className="w-full text-left text-xs text-[#0D0C22]">
            ...
            <tbody className="divide-y divide-black/5">
              {users.map((u) => (
                <tr key={u.id} className="hover:bg-[#F8F7F4] transition-colors">
                  <td className="p-4">
                    <p className="font-bold text-[#0D0C22]">{u.name}</p>
                    <p className="text-[10px] text-[#6E6D7A]">{u.email}</p>
                  </td>
                  <td className="p-4">
                    <button
                      ...
                      className={`text-[10px] font-black px-2.5 py-1 rounded-full border transition-all ${
                        u.role === 'ADMIN'
                          ...
                          : 'bg-[#F8F7F4] text-[#6E6D7A] border-black/10 hover:bg-[#FDE3E1]/60 hover:text-[#EA4C89]'
                      }`}
                    >
                      {u.role}
                    </button>
                  ...
                  <td className="p-4 font-black text-[#0D0C22] text-sm">
                    {u.credits} <span className="text-[10px] text-[#6E6D7A] font-semibold">kredit</span>
                  </td>
                  <td className="p-4 font-semibold text-[#6E6D7A]">{u.projectCount} Proyek</td>
      ...
      {selectedUser && (
        <div className="fixed inset-0 z-50 bg-[rgba(13,12,34,0.4)] backdrop-blur-md flex items-center justify-center p-4">
          <div className="rounded-3xl glass-panel p-6 sm:p-8 max-w-md w-full border border-black/5 shadow-2xl relative space-y-6">
            <button
              onClick={() => setSelectedUser(null)}
              className="absolute top-4 right-4 p-2 text-[#6E6D7A] hover:text-[#0D0C22] rounded-xl bg-[#F8F7F4] hover:bg-black/5 transition-colors"
            >
              <X className="w-4 h-4" />
            </button>

            <div>
              <h3 className="text-xl font-bold text-[#0D0C22] flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-[#EA4C89]" />
                <span>Tambah Kredit AI</span>
              </h3>
              <p className="text-xs text-[#6E6D7A] mt-1">
                Menambahkan kredit pemrosesan secara manual ke akun <strong className="text-[#0D0C22]">{selectedUser.name}</strong> ({selectedUser.email}).
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-[#0D0C22] block mb-2">Mode Penambahan</label>
                  ...
                  <button
                    ...
                    className={`py-2 px-3 rounded-xl text-xs font-bold border transition-all ${
                      creditMode === 'ADD'
                        ...
                        : 'bg-white text-[#6E6D7A] border-black/10 hover:bg-[#F8F7F4]'
                    }`}
                  >
                    Tambah (+ Kredit)
                  </button>
              ...
              <div>
                <label className="text-xs font-bold text-[#0D0C22] block mb-2">Jumlah Kredit</label>
                <input
                  type="number"
                  value={creditAmount}
                  onChange={(e) => setCreditAmount(parseInt(e.target.value) || 0)}
                  className="w-full bg-[#F8F7F4] border border-transparent rounded-xl px-4 py-2.5 text-[#0D0C22] text-sm font-bold focus:outline-none focus:border-[#EA4C89] focus:ring-1 focus:ring-[#EA4C89]"
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                {[30, 50, 100, 300].map((val) => (
                  <button
                    ...
                    className="flex-1 py-1.5 rounded-lg bg-[#F8F7F4] hover:bg-black/5 text-xs font-bold text-[#0D0C22] border border-black/10"
                  >
                    +{val}m
                  </button>
                ))}
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-black/10">
              <button
                type="button"
                onClick={() => setSelectedUser(null)}
                className="px-4 py-2.5 rounded-xl text-xs font-bold text-[#6E6D7A] hover:text-[#0D0C22] bg-[#F8F7F4] hover:bg-black/5 transition-colors"
              >
                Batal
              </button>
              <button
