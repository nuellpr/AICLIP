...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen text-white selection:bg-blue-500 selection:text-black relative bg-[#05060B]">
      ...
      <header className="flex md:hidden items-center justify-between p-4 border-b border-white/10 glass-panel sticky top-0 z-40">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/dashboard/new" className="bg-gradient-to-r from-blue-500 to-purple-500 p-2 rounded-xl text-black font-bold">
            <Sparkles className="h-4 w-4" />
          </Link>
          <button onClick={handleLogout} className="p-2 rounded-xl bg-white/10 text-gray-400 hover:text-red-400" title="Keluar">
        ...
        <aside className="w-68 h-[calc(100vh-2rem)] rounded-3xl glass-panel p-6 flex flex-col relative overflow-hidden border border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.5)]">
          ...
          <nav className="flex-1 space-y-3.5 z-10">
            ...
            <Link href="/dashboard/new" className="relative group block">
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 rounded-2xl blur-[3px] opacity-70 group-hover:opacity-100 transition-opacity"></div>
            ...
            {user?.role === 'ADMIN' && (
              <Link href="/dashboard/admin" className="flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-gray-300 hover:bg-white/10 hover:text-white transition-all hover:translate-x-1 group font-semibold text-sm">
                <Shield className="h-5 w-5 group-hover:text-blue-400 transition-colors" />
            ...
            <div className="flex items-center justify-between p-3 rounded-2xl bg-white/5 border border-white/10">
              <div className="flex items-center gap-3 overflow-hidden">
                {user?.image ? (
                  ...
                  <img src={user.image} alt={user.name || 'User'} className="h-9 w-9 rounded-xl object-cover border border-blue-400/30" />
                ) : (
                  <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center font-bold text-xs text-black flex-shrink-0">
                    {userInitial}
                  </div>
                )}
      ...
      <main className="flex-1 overflow-y-auto p-4 md:py-4 md:pr-4 md:pl-0 pb-20 md:pb-4">
        <div className="min-h-[calc(100vh-2rem)] md:h-[calc(100vh-2rem)] rounded-3xl glass-panel p-4 sm:p-6 md:p-8 overflow-y-auto overflow-x-hidden relative border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.4)]">
          ...
          <div className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full pointer-events-none -mr-40 -mt-40 bg-[radial-gradient(circle,rgba(37,99,235,0.05)_0%,transparent_70%)] will-change-transform"></div>
      ...
      <div className="fixed bottom-3 left-3 right-3 z-50 block md:hidden">
        <div className="glass-panel rounded-2xl p-2 flex items-center justify-around border border-white/15 bg-black/80 backdrop-blur-2xl shadow-[0_0_30px_rgba(0,0,0,0.8)]">
          <Link href="/dashboard" className="flex flex-col items-center gap-1 p-2 text-gray-400 hover:text-blue-400 transition-colors text-[10px] font-bold">
            <LayoutDashboard className="h-5 w-5" />
            Proyek
          </Link>
          <Link href="/dashboard/new" className="flex flex-col items-center gap-1 p-2 text-blue-400 font-bold text-[10px]">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-r from-blue-500 to-purple-500 text-black flex items-center justify-center -mt-6 border-2 border-black shadow-[0_0_15px_rgba(37,99,235,0.6)]">
          ...
          <Link href="/dashboard/billing" className="flex flex-col items-center gap-1 p-2 text-gray-400 hover:text-yellow-400 transition-colors text-[10px] font-bold">
