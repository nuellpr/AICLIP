...
import ThemeToggle from '@/components/clipforge/ThemeToggle';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen text-[var(--ink)] selection:bg-[#EA4C89]/25 selection:text-[var(--ink)] relative bg-[var(--db-cream)]">
      {/* Background Mesh Animation */}
      <div className="cf-mesh"></div>

      {/* Top Header for Mobile */}
      <header className="flex md:hidden items-center justify-between p-4 border-b border-[var(--db-line)] bg-[var(--db-panel)] sticky top-0 z-40">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleLogout} className="p-2 rounded-xl bg-[var(--db-cream)] text-[var(--db-gray)] hover:text-red-500 transition-colors" title="Keluar">
      ...
      <div className="p-4 flex-shrink-0 z-30 hidden md:block">
        <aside className="w-68 h-[calc(100vh-2rem)] rounded-3xl bg-[var(--db-panel)] p-6 flex flex-col relative overflow-hidden border border-[var(--db-line)] shadow-[0_10px_40px_-10px_rgba(13,12,34,0.1)]">
          ...
          <nav className="flex-1 space-y-3.5 z-10">
            <Link href="/dashboard" className="flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-[var(--db-gray)] hover:bg-[var(--db-cream)] hover:text-[var(--ink)] transition-all hover:translate-x-1 group font-semibold text-sm">
            ...
            <Link href="/dashboard/library" className="flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-[var(--db-gray)] hover:bg-[var(--db-cream)] hover:text-[var(--ink)] transition-all hover:translate-x-1 group font-semibold text-sm">
              <FolderKanban className="h-5 w-5 group-hover:text-[#EA4C89] transition-colors" />
              <span>Penyimpanan Klip</span>
            </Link>

            <Link href="/dashboard/settings" className="flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-[var(--db-gray)] hover:bg-[var(--db-cream)] hover:text-[var(--ink)] transition-all hover:translate-x-1 group font-semibold text-sm">
              <Settings className="h-5 w-5 group-hover:text-[#EA4C89] transition-colors" />
              <span>Pengaturan AI</span>
            </Link>

            <Link href="/dashboard/billing" className="flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-[var(--db-gray)] hover:bg-[var(--db-cream)] hover:text-[var(--ink)] transition-all hover:translate-x-1 group font-semibold text-sm">
              <CreditCard className="h-5 w-5 group-hover:text-[#EA4C89] transition-colors" />
              <span>Langganan & Tagihan</span>
            </Link>

            {user?.role === 'ADMIN' && (
              <Link href="/dashboard/admin" className="flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-[var(--db-gray)] hover:bg-[var(--db-cream)] hover:text-[var(--ink)] transition-all hover:translate-x-1 group font-semibold text-sm">
            ...
            <div className="rounded-2xl bg-[var(--db-cream)] p-4 relative overflow-hidden border border-[var(--db-line)]">
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div className="flex flex-col items-start gap-2">
                  ...
                  <p className="text-xs font-semibold text-[var(--db-gray)] leading-snug">kredit pemrosesan<br />AI tersisa</p>
                </div>
              </div>
            </div>

            {/* Quick User Badge */}
            <div className="flex items-center justify-between p-3 rounded-2xl bg-[var(--db-cream)] border border-[var(--db-line)]">
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-[var(--ink)] truncate">{user?.name || 'Demo User'}</p>
                  <p className="text-[10px] text-[var(--db-gray)] truncate">{user?.email || 'demo@clipforge.ai'}</p>
                </div>
              </div>
              <button
                onClick={handleLogout}
                className="text-[var(--db-gray)] hover:text-red-500 transition-colors p-1.5 rounded-lg hover:bg-[var(--db-panel)] flex-shrink-0"
                title="Keluar"
              >
                <LogOut className="h-4 w-4" />
              </button>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:py-4 md:pr-4 md:pl-0 pb-20 md:pb-4">
        <div className="min-h-[calc(100vh-2rem)] md:h-[calc(100vh-2rem)] rounded-3xl bg-[var(--db-panel)] p-4 sm:p-6 md:p-8 overflow-y-auto overflow-x-hidden relative border border-[var(--db-line)] shadow-[0_10px_40px_-10px_rgba(13,12,34,0.08)]">
      ...
      <div className="fixed bottom-3 left-3 right-3 z-50 block md:hidden">
        <div className="rounded-2xl p-2 flex items-center justify-around border border-[var(--db-line)] bg-[var(--db-panel)]/95 backdrop-blur-2xl shadow-[0_10px_40px_-10px_rgba(13,12,34,0.25)]">
          <Link href="/dashboard" className="flex flex-col items-center gap-1 p-2 text-[var(--db-gray)] hover:text-[#EA4C89] transition-colors text-[10px] font-bold">
          ...
          <Link href="/dashboard/library" className="flex flex-col items-center gap-1 p-2 text-[var(--db-gray)] hover:text-[#EA4C89] transition-colors text-[10px] font-bold">
            <FolderKanban className="h-5 w-5" />
            Klip
          </Link>
          <Link href="/dashboard/settings" className="flex flex-col items-center gap-1 p-2 text-[var(--db-gray)] hover:text-[#EA4C89] transition-colors text-[10px] font-bold">
            <Settings className="h-5 w-5" />
            Pengaturan
          </Link>
          <Link href="/dashboard/billing" className="flex flex-col items-center gap-1 p-2 text-[var(--db-gray)] hover:text-[#EA4C89] transition-colors text-[10px] font-bold">
    ...
    <div className="flex flex-col items-center shrink-0">
      <div className="relative" style={{ width: SIZE, height: SIZE }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-xl font-black text-[var(--ink)]" style={{ fontFamily: 'var(--font-space-grotesk), system-ui, sans-serif' }}>
            {value}
          </span>
        </div>
      </div>
      <span className="text-[10px] font-bold text-[var(--db-gray)] uppercase tracking-wider mt-1.5">Kredit</span>
    </div>
  );
}
